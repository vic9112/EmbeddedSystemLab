...
class GattServerDemo : ble::GattServer::EventHandler {
    ...
    const static uint16_t EXAMPLE_SERVICE_UUID         = 0xA240;
    const static uint16_t WRITABLE_CHARACTERISTIC_UUID = 0xA241;
...
public:
    ...
    void start(BLE &ble, events::EventQueue &event_queue)
    {
        ...
        printf("Example service added with UUID 0xA240\r\n");
        printf("Connect and write to characteristic 0xA241\r\n");
    }
...
private:
    ...
    virtual void onDataWritten(const GattWriteCallbackParams &params)
    {
        ...
        if ((params.handle == _writable_characteristic->getValueHandle()) && (params.len == 1)) {
            ...
            printf("%d", int(*(params.data)));
            for (int i = 0; i < int(*(params.data)); i++) {
