...
void uLCD_4DGL ::writeBYTE(char c) // send a BYTE command to screen
{
    ...
    ThisThread::sleep_for(TEMPO*1ms); //mbed is too fast for LCD at high baud rates in some long commands
...
void uLCD_4DGL ::writeBYTEfast(char c) // send a BYTE command to screen
{
    ...
    send[0]=c;
    _cmd.write(send, 1);
    //ThisThread::sleep_for(TEMPO*1ms);  //mbed is too fast for LCD at high baud rates - but not in short commands
