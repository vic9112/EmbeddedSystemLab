...
void playNoteD(void) { idD = queue.call_every(1ms, playNote, 147, 100); }
void playNoteF(void) { idF = queue.call_every(1ms, playNote, 175, 100); }
void playNoteA(void) { idA = queue.call_every(1ms, playNote, 220, 100); }
...
int main(void) {
  ...
  keyboard0.fall(queue.event(playNoteD));
  keyboard1.fall(queue.event(playNoteF));
  keyboard2.fall(queue.event(playNoteA));
  keyboard0.rise(queue.event(stopPlayNoteD));
  keyboard1.rise(queue.event(stopPlayNoteF));
  keyboard2.rise(queue.event(stopPlayNoteA));
  //loadWaveform();
  printWaveform();
}