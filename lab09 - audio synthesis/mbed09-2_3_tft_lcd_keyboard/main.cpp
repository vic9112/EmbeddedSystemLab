...
static void event_handler(lv_obj_t *obj, lv_event_t event) {
  ...
  for(int i=0; i<notes; i++){
    ...
    if(index!=-1){ //Check if notes index is supported otherwise do nothing
        //id[index]=queue.call_every(10ms, playNote, frequency[index], 0.01, 1); //Repeatively play a note with a duration of 0.01s or 10ms
        if (event == LV_EVENT_PRESSING) {
            id[index]=queue.call(playNote, frequency[index], 1, 1); //call playNote for 1s once        
